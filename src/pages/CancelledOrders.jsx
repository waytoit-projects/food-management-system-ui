...
import { 
  Search, Loader2, Calendar as CalendarIcon, XCircle, 
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div 
        ...
        style={{ 
          display: 'flex', alignItems: 'center', background: 'rgba(239, 68, 68, 0.1)', 
          border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '1.5rem', padding: '0.5rem 1.25rem', 
          color: '#ef4444', fontWeight: 700, letterSpacing: '0.02em', cursor: 'pointer', transition: 'all 0.2s'
        }}
        ...
        <>
          ...
          <div style={{ 
                ...
                return (
                  <button 
                    ...
                    style={{ 
                      ...
                      borderRadius: '50%', background: isSelected ? '#ef4444' : 'transparent',
                      ...
                      boxShadow: isSelected ? '0 4px 10px rgba(239, 68, 68, 0.4)' : 'none',
                      transition: 'all 0.2s'
                    }}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem' }}>
              ...
              <button onClick={() => setIsOpen(false)} style={{ flex: 1, padding: '0.6rem', borderRadius: '2rem', background: '#ef4444', color: 'white', border: 'none', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 10px rgba(239, 68, 68, 0.3)' }}>Done</button>
            </div>
          </div>
        </>
      )}
...
const CancelledOrders = () => {
  ...
  const handleExport = () => {
    ...
    link.setAttribute("download", `cancelled_orders_${selectedDate}.csv`);
  ...
  const fetchOrders = async () => {
    ...
    setLoading(true);
    try {
      const response = await getCancelledOrders(selectedDate);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      dineIn: new Set(cancelled.filter(o => (o.given_type || o.givenType)?.toUpperCase().includes('DINE')).map(o => o.orderId)).size,
      delivery: new Set(cancelled.filter(o => (o.given_type || o.givenType)?.toUpperCase().includes('DELIVERY')).map(o => o.orderId)).size,
      takeaway: new Set(cancelled.filter(o => (o.given_type || o.givenType)?.toUpperCase().includes('TAKEAWAY')).map(o => o.orderId)).size,
    };
  }, [ordersData]);
  ...
  return (
    <div style={{ padding: '1rem', height: '100%', width: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, margin: '0 0 0.5rem 0', background: 'linear-gradient(90deg, #ff4d4d, #f97316)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Cancelled Orders
          </h1>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Review and analyze orders that were cancelled.</p>
        </div>
        <div style={{ display: 'flex', background: 'var(--bg-card)', padding: '0.4rem', borderRadius: '2rem', border: '1px solid var(--border-color)', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button onClick={fetchOrders} disabled={loading} style={{ padding: '0.5rem 1.5rem', borderRadius: '1.5rem', background: '#ef4444', color: 'white', border: 'none', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(239, 68, 68, 0.3)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
        {[
          { label: 'Cancelled Orders', count: stats.total, icon: XCircle, color: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)' },
          { label: 'Total Items Lost', count: stats.items, icon: Package, color: '#f97316', bg: 'rgba(249, 115, 22, 0.1)' },
          { label: 'Dine In (Cancelled)', count: stats.dineIn, icon: Utensils, color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)' },
          { label: 'Delivery (Cancelled)', count: stats.delivery, icon: MapPin, color: '#8b5cf6', bg: 'rgba(139, 92, 246, 0.1)' },
          { label: 'Takeaway (Cancelled)', count: stats.takeaway, icon: RefreshCcw, color: '#ec4899', bg: 'rgba(236, 72, 153, 0.1)' },
        ].map((stat, idx) => (
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '1.25rem', border: '1px solid var(--border-color)', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <button onClick={handleExport} disabled={sortedData.length === 0} style={{ padding: '0.4rem 1rem', borderRadius: '0.5rem', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Download size={14} /> Export CSV
            </button>
          </div>
          <div style={{ position: 'relative' }}>
            <Search size={14} color="var(--text-muted)" style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)' }} />
            <input type="text" placeholder="Search cancelled..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} style={{ padding: '0.4rem 0.75rem 0.4rem 2rem', borderRadius: '0.5rem', border: '1px solid var(--border-color)', background: 'rgba(255,255,255,0.02)', color: 'white', width: '180px', fontSize: '0.8rem', outline: 'none' }} />
          </div>
        </div>

        {loading ? (
          <div style={{ padding: '4rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <Loader2 size={40} color="#ef4444" className="spinner" />
          </div>
        ) : (
              ...
              <tbody>
                {currentItems.length > 0 ? currentItems.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '1rem 0.75rem', color: '#ef4444', fontWeight: 700, whiteSpace: 'nowrap' }}>{item.quantity}</td>
                  ...
                  <tr><td colSpan="29" style={{ padding: '4rem', textAlign: 'center', color: 'var(--text-muted)' }}>No cancelled orders found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
