...
const AllOrders = () => {
  ...
  const handleExport = () => {
    ...
    const headers = [
      "Order ID", "Item ID", "Item Name", "Category", "Qty", 
      "Item Price", "Discount", "Selling Amt", "GST", "Total Amt", 
      "Payment Mode", "Pay Status", "Transaction ID", "Voucher ID",
    ...
    link.setAttribute("download", `all_orders_${selectedDate}.csv`);
  ...
  return (
    <div style={{ padding: '1rem', height: '100%', width: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, margin: '0 0 0.5rem 0', background: 'linear-gradient(90deg, #6366f1, #8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            All Orders
          </h1>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Exhaustive view of all orders (Completed, Cancelled, & Pending).</p>
        </div>
        <div style={{ display: 'flex', background: 'var(--bg-card)', padding: '0.4rem', borderRadius: '2rem', border: '1px solid var(--border-color)', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button onClick={fetchOrders} disabled={loading} style={{ padding: '0.5rem 1.5rem', borderRadius: '1.5rem', background: '#6366f1', color: 'white', border: 'none', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(99, 102, 241, 0.3)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem' }}>
        {[
          { label: 'Total Orders', count: stats.total, icon: List, color: '#6366f1', bg: 'rgba(99, 102, 241, 0.1)' },
          { label: 'Total Items', count: stats.items, icon: Package, color: '#8b5cf6', bg: 'rgba(139, 92, 246, 0.1)' },
          { label: 'Pending', count: stats.pending, icon: Clock, color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)' },
          { label: 'Completed', count: stats.completed, icon: Utensils, color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)' },
          { label: 'Cancelled', count: stats.cancelled, icon: RefreshCcw, color: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)' },
        ].map((stat, idx) => (
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '1.25rem', border: '1px solid var(--border-color)', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <button onClick={handleExport} disabled={sortedData.length === 0} style={{ padding: '0.4rem 1rem', borderRadius: '0.5rem', background: 'rgba(99, 102, 241, 0.1)', color: '#6366f1', border: '1px solid rgba(99, 102, 241, 0.2)', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Download size={14} /> Export CSV
            </button>
          </div>
          <div style={{ position: 'relative' }}>
            <Search size={14} color="var(--text-muted)" style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)' }} />
            <input type="text" placeholder="Search all orders..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} style={{ padding: '0.4rem 0.75rem 0.4rem 2rem', borderRadius: '0.5rem', border: '1px solid var(--border-color)', background: 'rgba(255,255,255,0.02)', color: 'white', width: '180px', fontSize: '0.8rem', outline: 'none' }} />
          </div>
        </div>

        {loading ? (
          <div style={{ padding: '4rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <Loader2 size={40} color="#6366f1" className="spinner" />
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              ...
              <tbody>
                {currentItems.length > 0 ? currentItems.map((item, idx) => {
                  ...
                  <tr><td colSpan="29" style={{ padding: '4rem', textAlign: 'center', color: 'var(--text-muted)' }}>No orders found for this date.</td></tr>
                )}
              </tbody>
            </table>
          </div>
