...
import { 
  Search, Loader2, Calendar as CalendarIcon, List, Clock,
  MapPin, Package, RefreshCcw, Utensils, ChevronLeft, ChevronRight, ArrowUpDown, Download, Receipt
} from 'lucide-react';
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div 
        ...
        style={{ 
          display: 'flex', alignItems: 'center', background: 'rgba(99, 102, 241, 0.1)', 
          border: '1px solid rgba(99, 102, 241, 0.2)', borderRadius: '1.5rem', padding: '0.5rem 1.25rem', 
          color: '#818cf8', fontWeight: 700, letterSpacing: '0.02em', cursor: 'pointer', transition: 'all 0.2s'
        }}
        ...
        <>
          ...
          <div style={{ 
                ...
                return (
                  <button 
                    ...
                    style={{ 
                      ...
                      borderRadius: '50%', background: isSelected ? '#6366f1' : 'transparent',
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem' }}>
              ...
              <button onClick={() => setIsOpen(false)} style={{ flex: 1, padding: '0.6rem', borderRadius: '2rem', background: '#6366f1', color: 'white', border: 'none', fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 10px rgba(99, 102, 241, 0.3)' }}>Done</button>
            </div>
          </div>
        </>
      )}
...
const TotalBills = () => {
  ...
  const handleExport = () => {
    ...
    const headers = [
      "Bill ID", "Item ID", "Item Name", "Category", "Qty", 
    ...
    const csvContent = [
      headers.join(","),
      ...sortedData.map(item => [
        item.orderId, item.itemId, item.itemName, item.itemCategory, item.quantity,
        item.itemPrice, item.discountAmount, item.sellingAmount, item.gstAmount, item.totalAmount,
        item.paymentMode, item.paymentStatus, item.transactionId, item.voucherCardId,
        item.customerName || 'Walk-in', item.customerMobile || '', item.tableNo, item.deliveryAddress || '',
        item.orderDate, item.orderTime, item.insertedBy, item.insertedTime,
        item.updatedBy || '', item.updatedTime || '', item.hotelName, item.hotelAddress,
        item.remarks || '', item.orderStatus
      ].map(val => `"${val}"`).join(","))
    ].join("\n");
    ...
    link.setAttribute("download", `total_bills_${selectedDate}.csv`);
  ...
  const fetchOrders = async () => {
    ...
    setLoading(true);
    try {
      const response = await getAllOrders(selectedDate);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      completed: new Set(completed.map(o => o.orderId)).size,
      cancelled: new Set(cancelled.map(o => o.orderId)).size,
      pending: new Set(pending.map(o => o.orderId)).size,
      totalRevenue: completed.reduce((sum, item) => sum + (item.totalAmount || 0), 0)
    };
  }, [ordersData]);
  ...
  return (
    <div style={{ padding: '1rem', height: '100%', width: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, margin: '0 0 0.5rem 0', background: 'linear-gradient(90deg, #10b981, #3b82f6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Total Orders Bills
          </h1>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Comprehensive list of all generated bills and financial records.</p>
        </div>
        <div style={{ display: 'flex', background: 'var(--bg-card)', padding: '0.4rem', borderRadius: '2rem', border: '1px solid var(--border-color)', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button onClick={fetchOrders} disabled={loading} style={{ padding: '0.5rem 1.5rem', borderRadius: '1.5rem', background: '#10b981', color: 'white', border: 'none', fontWeight: 700, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: '0 4px 10px rgba(16, 185, 129, 0.3)' }}>
            {loading ? <Loader2 size={16} className="spinner" /> : 'Refresh Bills'}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1rem' }}>
        {[
          { label: 'Total Bills', count: stats.total, icon: Receipt, color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)' },
          { label: 'Revenue', count: `₹${stats.totalRevenue.toFixed(2)}`, icon: Download, color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)' },
          { label: 'Paid Bills', count: stats.completed, icon: Utensils, color: '#10b981', bg: 'rgba(16, 185, 129, 0.1)' },
          { label: 'Pending Bills', count: stats.pending, icon: Clock, color: '#f59e0b', bg: 'rgba(245, 158, 11, 0.1)' },
          { label: 'Cancelled Bills', count: stats.cancelled, icon: RefreshCcw, color: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)' },
        ].map((stat, idx) => (
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '1.25rem', border: '1px solid var(--border-color)', overflow: 'hidden', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <button onClick={handleExport} disabled={sortedData.length === 0} style={{ padding: '0.4rem 1rem', borderRadius: '0.5rem', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', border: '1px solid rgba(16, 185, 129, 0.2)', fontWeight: 600, fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <Download size={14} /> Export CSV
            </button>
          </div>
          <div style={{ position: 'relative' }}>
            <Search size={14} color="var(--text-muted)" style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)' }} />
            <input type="text" placeholder="Search bills..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} style={{ padding: '0.4rem 0.75rem 0.4rem 2rem', borderRadius: '0.5rem', border: '1px solid var(--border-color)', background: 'rgba(255,255,255,0.02)', color: 'white', width: '180px', fontSize: '0.8rem', outline: 'none' }} />
          </div>
        </div>

        {loading ? (
          <div style={{ padding: '4rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            <Loader2 size={40} color="#10b981" className="spinner" />
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead style={{ position: 'sticky', top: 0, zIndex: 10, background: '#1e293b' }}>
                <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                  {[
                    '#', 'Bill ID', 'Item Name', 'Category', 'Qty', 
                    'Selling Amt', 'GST', 'Total Amt', 
                    'Payment Mode', 'Pay Status', 'Order Type',
                    'Customer', 'Table', 'Date', 'Time', 'Inserted By', 'Status'
                  ].map((h, i) => (
                ...
                {currentItems.length > 0 ? currentItems.map((item, idx) => {
                  ...
                  return (
                    <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '1rem 0.75rem', color: '#818cf8', fontWeight: 700, whiteSpace: 'nowrap' }}>{item.quantity}</td>
                  ...
                  <tr><td colSpan="17" style={{ padding: '4rem', textAlign: 'center', color: 'var(--text-muted)' }}>No bills found for this date.</td></tr>
                )}
              </tbody>
            </table>
          </div>
